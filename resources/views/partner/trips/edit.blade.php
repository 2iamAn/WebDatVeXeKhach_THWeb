...
<div class="page-header">
    ...
    <p class="text-muted mb-0 mt-2">Cập nhật thông tin chuyến xe</p>
</div>
...
<div class="card shadow-sm border-0" style="border-radius: 15px;">
    <div class="card-body p-4">
        ...
        <form method="POST" action="{{ route('partner.trips.update', $chuyen->MaChuyenXe) }}" class="row g-3">
            @csrf
            
            @if($chuyen->TrangThai == 'TuChoi' && $chuyen->LyDoTuChoi)
                <div class="col-12">
                    <div class="alert alert-danger">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        <strong>Lý do từ chối:</strong> {{ $chuyen->LyDoTuChoi }}
                    </div>
                </div>
            @endif
            ...
            <div class="col-md-6">
                ...
                <select name="MaTuyen" class="form-control @error('MaTuyen') is-invalid @enderror" 
                    ...
                    @foreach($tuyens as $tuyen)
                        <option value="{{ $tuyen->MaTuyen }}" {{ old('MaTuyen', $chuyen->MaTuyen) == $tuyen->MaTuyen ? 'selected' : '' }}>
            ...
            <div class="col-md-6">
                ...
                <select name="MaXe" class="form-control @error('MaXe') is-invalid @enderror" 
                    ...
                    @foreach($xes as $xe)
                        <option value="{{ $xe->MaXe }}" {{ old('MaXe', $chuyen->MaXe) == $xe->MaXe ? 'selected' : '' }}>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="DiemLenXe" class="form-control @error('DiemLenXe') is-invalid @enderror" 
                       value="{{ old('DiemLenXe', $chuyen->DiemLenXe ?? '') }}" 
                       required
                       style="border-radius: 10px; padding: 12px;"
                       placeholder="Ví dụ: Bến xe An Sương...">
                @error('DiemLenXe')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="DiemXuongXe" class="form-control @error('DiemXuongXe') is-invalid @enderror" 
                       value="{{ old('DiemXuongXe', $chuyen->DiemXuongXe ?? '') }}" 
                       required
                       style="border-radius: 10px; padding: 12px;"
            ...
            <div class="col-md-6">
                ...
                <input type="datetime-local" name="GioKhoiHanh" 
                       class="form-control" 
                       value="{{ optional($chuyen->GioKhoiHanh)->format('Y-m-d\TH:i') }}" 
                       required
                       style="border-radius: 10px; padding: 12px;">
            </div>
            ...
            @if($chuyen->TrangThai == 'TuChoi')
                <div class="col-12">
                    <div class="alert alert-info">
                        ...
                        <strong>Lưu ý:</strong> Sau khi chỉnh sửa, chuyến xe sẽ tự động được gửi lại để admin phê duyệt.
                    </div>
                </div>
            @endif
